$(function(){
    window.cars ={
        ...
        getOptions: function(){
  			//alert(this.selectedCarYear + " " + this.selectedModel + " " + this.selectedPart);
	...
	if($("#preload-ppc").val()=="true") { // preloading form values upon initial page load when needed
		...
		if ($('#preload-model').val()!=undefined&&$('#preload-model').val()!="") {
			...
			if ($('#preload-make').val()==undefined||$('#preload-make').val()=="") {
				...
				//cars.selectedManufacture = "Acura";
...
function preload_year() {
	...
	if(($('#preload-model').val()!=undefined&&$('#preload-model').val()!=""))
		var params = { carYears:1,carYearsModel:$('#preload-model').val() };
...
function preload_model() {
	...
	if ($('#preload-model').val()!=undefined&&$('#preload-model').val()!="")
		$('#group-model .select2-chosen').text(cars.selectedCarModel);
	else
		$('#group-model .select2-chosen').text("Model");
}
...
function preload_part() {
	if(cars.selectedCarYear)
		...
		var parts = cars.getCarsData({carModel:cars.selectedCarModel});
	...
	if(parts!="")
	{
		...
		for(var p in parts)
		{
			if(parts[p].part.id==$('#preload-part').val()) {
