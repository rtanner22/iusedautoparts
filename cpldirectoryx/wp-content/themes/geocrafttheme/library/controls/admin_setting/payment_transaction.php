...
function geocraft_transaction_opt() {
    ...
    $query = $wpdb->query($query); // Get total of Num rows from the database query
    ...
    if ($pn == 1) {
        ...
        $centerPages .= '&nbsp; <span class="active">' . $pn . '</span> &nbsp;';
        $centerPages .= '&nbsp; <a href="' . admin_url("/admin.php?page=transaction&pn=$add1") . '">' . $add1 . '</a> &nbsp;';
        $centerPages .= '&nbsp; <a href="' . admin_url("/admin.php?page=transaction&pn=$add1") . '">' . $add2 . '</a> &nbsp;';
    ...
    if ($lastPage != "1") {
        ...
        if ($pn != $lastPage) {
            $nextPage = $pn + 1;
            $paginationDisplay .= '&nbsp;  <a href="' . admin_url("/admin.php?page=transaction&pn&pn=$nextPage") . '"> Next</a> ';
        }
    }
    }
    ?>
    <div class="wrap" id="of_container" style="width:1050px;">
        ...
        <table id="tblspacer" class="widefat fixed">
            ...
            <?php
            global $wpdb, $transection_table_name;
            $querys = "SELECT * FROM $transection_table_name ORDER BY trans_id ASC $limit";
            $results = $wpdb->get_results($querys);
            if ($results):
                ?>
                <tbody id="trans_list">
                    <?php
                    foreach ($results as $result):
                        ...
                        <tr>
                            ...
                            <td><?php echo $result->txn_type; ?></td>
