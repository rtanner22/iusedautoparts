<?php
    ...
    try {
        ...
        if (isset($interchange[2])) {
            mysql_query("INSERT INTO inventory_r (yardid,inventoryid,inventorynumber,quantityavailable,stockticketnumber,modelyear,modelname,conditionsandoptions,mileage,conditioncode,partrating,wholesaleprice) SELECT yardid,inventoryid,inventorynumber,quantityavailable,stockticketnumber,modelyear,modelname,conditionsandoptions,mileage,conditioncode,partrating,wholesaleprice FROM inventory WHERE inventorynumber = '" . $interchange[2] . "';");
        }
    } catch (Exception $exc) {}
